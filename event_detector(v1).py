...
class EventDetector:
    def __init__(self, model_path):
        ...
        try:
            ...
            self.model.eval()
            print(f"✅ Event detector model loaded: {model_path}")
        except Exception as e:
            print(f"⚠️  Error loading event detector model: {e}")
            raise
    ...
    def predict(self, frame, frame_num):
        try:
            ...
            # DEBUG: print predictions every 60 frames to see model behavior
            if frame_num % 60 == 0:
                print(f"[DEBUG] Frame {frame_num}: {event_name} ({conf:.2f})")

            # Return event if it's not 'none' (let event_logic handle confidence thresholds)
            if event_name != "none":
                print(f"🔴 EVENT DETECTED at frame {frame_num}: {event_name} (confidence: {conf:.2f})")
                return {
                    "frame": frame_num,
                    "event": event_name,
                    "confidence": conf
